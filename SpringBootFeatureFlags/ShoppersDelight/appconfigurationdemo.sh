...
addSegments()
{
	...
	segmentStatus=$(curl -s --write-out 'HTTPSTATUS:%{http_code}'  -X POST $segmentUpdateURL -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" --data '{"segment_id": "'"${segmentId}"'", "name": "Prime customers","description": "Segment defining the paid customers of shoppers delight application","tags": "paid users","rules" :  [{"values":["shoppers.com"],"operator":"contains","attribute_name":"email"}]}' )
